...
class GGNN(nn.Module):
    ...
    def forward(self, input):
        ...
        for t in range(self.time_step):
            # eq(2)
            a = torch.bmm(batch_in_matrix.to(input.device), batch_aog_nodes.to(input.device))
            b = torch.bmm(batch_out_matrix.to(input.device), batch_aog_nodes.to(input.device))
            av = torch.cat((a, b), 2)
