...
from networks.densenet_ import densenet121
from semantic import semantic
from ggnn import GGNN
from element_wise_layer import Element_Wise_Layer

class Densenet_GNN(nn.Module):
    def __init__(self, image_feature_dim, output_dim, time_step,
        ...
        super(Densenet_GNN, self).__init__()
    ...
    def forward(self, x):
        batch_size = x.size()[0]
        img_feature_map = self.densenet121(x)
        graph_net_input = img_feature_map #--->(batch_size, classiffical, features)-->(, 3, 1024)
