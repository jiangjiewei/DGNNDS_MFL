...
from utils.load_pretrain_model import load_pretrain_model_dense
from utils.metrics import voc12_mAP, AveragePrecisionMeter
from densenet_GNN import Densenet_GNN, Densenet_NGNN
from models_LSTM import SSGRL_LSTM, SSGRL_SD
...
def main():
    ...
    elif args.model_name == "densenet_NGNN":
        model = Densenet_NGNN(image_feature_dim=1024,
                           output_dim=1024, time_step=3,
                           word_feature_dim=args.word_feature_dim,
                           adjacency_matrix=args.graph_file,
                           word_features=args.word_file,
                           num_classes=args.num_classes)
    elif args.model_name == "densenet_GNN":
        model = Densenet_GNN(image_feature_dim=1024,
                         output_dim=1024, time_step=3,
                         word_feature_dim=args.word_feature_dim,
                         adjacency_matrix=args.graph_file,
                         word_features=args.word_file,
    ...
    if args.pretrained:
        if args.model_name == "densenet121":
            ...
            model.classifier = nn.Linear(inchannel, 1000)
            model.load_state_dict({re.sub(r'\.(\d)\.', r'\1.', k): v for k, v in torch.load(args.pretrain_model).items()})
            inchannel = model.classifier.in_features
            model.classifier = nn.Linear(inchannel, args.num_classes)
        else:
            model = load_pretrain_model_dense(model, args)
    ...
    if args.model_name != "densenet121":
        if torch.cuda.device_count() > 1:
            ...
            for p in model.module.densenet121.parameters():
                p.requires_grad = False
            for p in model.module.densenet121.features.denseblock4.parameters():
                p.requires_grad = True
        else:
            for p in model.densenet121.parameters():
                p.requires_grad = False
            for p in model.densenet121.features.denseblock4.parameters():
                p.requires_grad = True
            model.cuda()
    else:
        if torch.cuda.device_count() > 1:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    if args.model_name == "densenet121":
        ...
        if torch.cuda.device_count() > 1:
            model.module.densenet121.eval()
            model.module.densenet121.features.denseblock4.train()
        else:
            model.densenet121.eval()
            model.densenet121.features.denseblock4.train()
